...
export default function GolfApp() {
  ...
  const runningTotal = scores.reduce((acc, s, i) => 
    i <= currentHole ? acc + calcHolePoints(s, courseData[i].par, courseData[i].si) : acc, 0
  );

  const finalTotal = scores.reduce((acc, s, i) => 
    acc + calcHolePoints(s, courseData[i].par, courseData[i].si), 0
  );
      ...
      <div style={containerStyle}>
        ...
        <div style={{ width: '100%', backgroundColor: 'white', color: '#333', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', overflow: 'hidden' }}>
          ...
          <div style={{ textAlign: 'center', padding: '5px 0' }}>
            <p style={{ fontSize: '22px', fontWeight: '900', color: '#999', margin: '0 0 5px 0' }}>STROKES</p>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}>
              ...
              <span style={{ fontSize: '140px', fontWeight: '900', color: '#1A4D3A', lineHeight: 0.75 }}>{scores[currentHole] === 0 ? "X" : scores[currentHole]}</span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                <button onClick={pickUp} style={{ width: '80px', height: '45px', backgroundColor: '#666', color: 'white', borderRadius: '10px', fontSize: '24px', fontWeight: '900', border: 'none' }}>PU</button>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around', backgroundColor: '#e8f5e9', padding: '2px 0' }}>
            <div style={{ textAlign: 'center' }}><p style={{ fontSize: '20px', color: '#16a34a', fontWeight: '900', margin: 0 }}>PTS</p><p style={{ fontSize: '70px', fontWeight: '900', lineHeight: 0.8, margin: 0 }}>{calcHolePoints(scores[currentHole], courseData[currentHole].par, courseData[currentHole].si)}</p></div>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', gap: '4px', padding: '2px 4px 45px 4px' }}>
            <button onClick={() => setCurrentHole(Math.max(0, currentHole - 1))} style={{ flex: 1, padding: '10px 0', backgroundColor: '#666', color: 'white', fontSize: '20px', fontWeight: 'bold', border: 'none', borderRadius: '10px', visibility: currentHole === 0 ? 'hidden' : 'visible' }}>BACK</button>
            {currentHole < 17 ? (
              <button onClick={() => setCurrentHole(currentHole + 1)} style={{ flex: 2, padding: '10px 0', backgroundColor: '#C9A66B', color: 'white', fontSize: '28px', fontWeight: '900', border: 'none', borderRadius: '10px' }}>NEXT HOLE</button>
            ) : (
              <button onClick={() => setShowSummary(true)} style={{ flex: 2, padding: '10px 0', backgroundColor: '#2563eb', color: 'white', fontSize: '28px', fontWeight: '900', border: 'none', borderRadius: '10px' }}>REVIEW</button>
            )}
          </div>
        </div>
      </div>
